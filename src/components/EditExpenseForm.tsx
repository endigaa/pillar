...
export function EditExpenseForm({ initialValues, onSubmit, onFinished, areas = [], projectId }: EditExpenseFormProps) {
  ...
  const form = useForm<ExpenseFormValues>({
    ...
    defaultValues: {
      ...
      amount: initialValues.amount / 100, // Convert cents to dollars
      date: new Date(initialValues.date),
      category: initialValues.category,
      workStage: initialValues.workStage || '',
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-6">
          ...
          <div className="flex justify-end space-x-2 pt-4">
              ...
              <Button type="submit" disabled={isSubmitting || isLoading}>{(isSubmitting || isLoading) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Update Expense</Button>
          </div>
        </form>
      </Form>
