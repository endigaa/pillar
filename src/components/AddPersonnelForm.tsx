...
interface AddPersonnelFormProps {
  onSubmit: (values: Omit<Personnel, 'id' | 'associatedExpenseIds'>) => Promise<void>;
  onFinished: () => void;
}
export function AddPersonnelForm({ onSubmit, onFinished }: AddPersonnelFormProps) {
  ...
  const handleFormSubmit = async (values: PersonnelFormValues) => {
    const personnelData = {
      ...
      nextOfKin: values.nextOfKin.map(nok => ({ ...nok, id: '' })), // Add dummy id to satisfy type
